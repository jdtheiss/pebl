function [fnd,C,S,reps]=pebl_find(fun,search,varargin)
% [fnd,C,S,reps]=pebl_find(fun,search,varargin)
...
% varargin - inputs for pebl_getfield (i.e., A, irep, itag)
%
% Outputs:
% fnd - a numeric array of logicals where the search was true (relating to 
% indices of pebl_getfield(varargin{:})).
...
% [fnd,C,S,reps] = pebl_find(@strcmp,'Control',sa,'ddt','\.group\{\d+\}$')
% fnd = 
%      1     0     0     0     0     1     0     0     1
% C = 
...
% [fnd,C,S,reps] = pebl_find(@strfind,'Results',h,'expr','\.String$')
% fnd =
%      0    1
% C = 
%     'Results:'
...
% [] is input as the third varargin (itag), pebl_find will use pebl_getfield to
...
% getfield
[C,S,reps] = pebl_getfield(varargin{:});
