function matlabbatch = sawa_setdeps(prebatch, matlabbatch)
...
% find cfg_dep
clear val1 tag1; 
[~,val1,tag1] = sawa_find(@isa,'cfg_dep',prebatch{m},'','');
...
n0 = subidx(strfind(sname,':'),'(end)'); n = subidx(strfind(sname,' '),'(end)'); 
% find module and snames that match
x0 = find(~cellfun('isempty',regexp(str1,regexptranslate('escape',sname(1:n0-1)))),1,'first'); 
tmp = sout1{x0};
...
x0 = find(~cellfun('isempty',regexp(str2,regexptranslate('escape',sname(1:n0-1)))),1,'first'); 
tmp = sout2{x0};
