...
% pval, kval, maskchc, and sep must be input in order (e.g., if pval does
% not exist and sep is entered as .5, pval would errantly be set to .5).
%
% requires: sawa_cat, sawa_screencapture, subidx, xjview
%
...
case 'char'
if exist(varargin{v},'file')==2
files = varargin(v); % files
elseif any(strcmp({'othermask','notxt','noxls','noimg','noslc'},varargin{v}))
...
if ~exist('othermask','var'), othermask = 0; end;
...
% get handles for different objects
xj_fig = findobj('-regexp','name','xjView.*'); hload = findobj(xj_fig,'string','Load Image'); 
...
if ~othermask, % not  othermask, set mask
set(hmsk,'value',find(strcmp(masks,mask))); cb = get(hmsk,'Callback'); cb(hmsk,[]);
else % if other mask, set first time
cb = get(homsk,'Callback'); cb(homsk,[]);    
end
...
xjimg = ['xjview_p' num2str(pval) 'k' num2str(kval) '_' ifil itype];
...
report = strjoin(rows(1:n(1)-5),'\n');
for i = 1:numel(n)
report = strjoin({report,rows(n(i)-4:n(i)),cell2strtable(cat(2,vols{i}',pct{i}',names{i}'),' ')},'\n');
end
...
% save slcimg
slcimg = [subidx(get(htcs,'string'),['{' num2str(x) '}']) '_p' num2str(pval) 'k' num2str(kval) '_' ifil itype];
