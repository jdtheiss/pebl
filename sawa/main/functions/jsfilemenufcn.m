function jsfilemenufcn(varargin)
...
if nargin == 4 % saves with name and type
    hfig = varargin{1};
    sname = varargin{3};
    stype = varargin{4};
    localSave(hfig, sname, stype);
elseif nargin == 2
    if ischar(hfig)
        cmd = hfig;
        hfig = gcbf;
...
% --------------------------------------------------------------------
function localSave(varargin)
if nargin > 1
    hfig = varargin{1};
    filename = varargin{2};
    types = localExportTypes;
    typevalue = localGetTypeFromExtension(varargin{3});
